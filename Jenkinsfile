   ...
   stages {
        	...
        	steps {
					...
					container_Up = false
                    			timeout(time: 1, unit: "HOURS") { 
                        			sh "curl -u admin:admin -X GET -H \"Accept: application/json\" http://104.248.169.167:9000/api/qualitygates/project_status?projectKey=com.mycompany:jenkinsnew > status.json"
      		...
      		steps {
        		script {
			  	//sh "docker stop apiops-anypoint-jenkins-sapi" 
			   	...
			   	LAST_STARTED = env.STAGE_NAME
				container_Up = false
			   	sh "docker build -t jenkinsnew:mule -f Dockerfile ."
      ...
      stage("Run container") {
      		steps {
        		script {
			     	LAST_STARTED = env.STAGE_NAME
          		    	sh " docker run -itd -p 8082:8081 --name jenkinsnew jenkinsnew:mule"
				container_Up = true
				sh "sleep 60"
       			}
		}
     ...
     stage ("Munit Test"){
			...
			script {
				...
				publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: "jenkinsnew/target/site/munit/coverage", reportFiles: "summary.html", reportName: "Munit coverage Report", reportTitles: ""])
				}
			}		
        	}    
     }
 ...
 //   }
    ...
    stage("Archetype"){
			...
			script {
					...
					def data = "archetype.artifactId=generic-db-sapi"
		    			...
		    			sh "mvn -f jenkinsnew/target/generated-sources/archetype/pom.xml -s $settings clean deploy -DaltSnapshotDeploymentRepository=nexus-snapshots::http://104.248.169.167:8081/repository/maven-snapshots/"
                  		} 
			}
              }   
     }
    ...
    stage("Deploy to Cloudhub"){
			...
			script {
				...
				sh "mvn -f pom.xml -s $settings package deploy -DmuleDeploy -DskipTests -Dkey=mymulesoft -Danypoint.username=njcdemo1 -Danypoint.password=Njclabs@123 -DapplicationName=jenkinsnew-mule -Dcloudhub.region=us-east-2 -Danypoint.platform.client_id=2873c31e7152405e9dc38600007108e8 -Danypoint.platform.client_secret=70da4Da4228749B7A33C848E9a3C0849"
				}
			}
             	}
    }
    ...
    stage("Kill container") {
      		steps {
        		script {
	  	        	LAST_STARTED = env.STAGE_NAME		
          		    	sh "docker rm -f jenkinsnew"
				sh "sleep 10"
				sh "docker rmi jenkinsnew:mule"
